...
for sat in ["S1_ASC", "S2"]:
    print(sat)
    if sat == "S1_ASC":
        b, t, c, h, w = 2, 20, 4, 64, 64    # 3 bands (VV, VH, VH/VV) + 1 angle
    else:
        b, t, c, h, w = 2, 20, 16, 64, 64   # 10 bands + 6 angles
    ...
    meteo = torch.randint(low=0, high=10, size=(b, t, 8, 6, h, w))
    meteo = meteo.float().nanmean((-2, -1))  # We compute spatial mean

    dem = torch.randint(low=0, high=10, size=[b, 1, 4, h, w])
